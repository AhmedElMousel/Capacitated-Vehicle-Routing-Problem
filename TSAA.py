...
for i in range(Runs):
    ...
    if len(Save_Solutions_Here) >= 1:
        ...
        if Solution_in_Hand in Tabu_List:  # If the solution is already in the tabu list
            while Solution_in_Hand[0] in Tabu_List[:, 0]:  # The distance of the solution is the same as one in the tabu
                if Solution_in_Hand[0] < Best_So_Far[0]:  # If it is less, then...
                    Solution_in_Hand = Best_So_Far  # It becomes best so far
                    break  # Then break
                else:
                    Solution_in_Hand = OF_Values_all_N_Ordered[
                        t]  # If it wasn't less, the current solution becomes what was chosen in the list
        ...
        if len(Tabu_List) >= Length_of_Tabu_List:  # If the tabu list was full
            ...
            if Tabu_List[0, 0] != Solution_in_Hand[0]:
                ...
                Tabu_List[0, :] = Solution_in_Hand[
                    np.newaxis]  # The solution in hand becomes instead of the first element
        else:
            Tabu_List = np.vstack(
                (Solution_in_Hand, Tabu_List))  # If the tabu list was not full, put the solution in hand in it

    Save_Solutions_Here = np.vstack(
        (Solution_in_Hand, Save_Solutions_Here))  # Save the solution in hand in a list to keep track of it
