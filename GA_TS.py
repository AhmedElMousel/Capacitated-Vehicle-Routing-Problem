# -*- coding: utf-8 -*-
"""
Created on Thu Jul  2 00:38:48 2020
...
class GATSPopulation(object):
    ...
    def step(self):
        ...
        self.iters += 1
        if not self.chromosomes[0][1].cost in self.tabulist:        
            if self.chromosomes[0][1].cost < self.best_solution.cost:
                self.best_solution = self.chromosomes[0][1]
                self.tabulist.append(self.chromosomes[0][1].cost)
        if len(self.tabulist) >= self.maxtabulen :
            del self.tabulist[0]
        if self.iters % 40 ==0 :
            self.maxtabulen = random.randrange(10,21)
...
class CVRPGATS(CVRPAlgorithm):
    def __init__(self, info, num_populations, total_iters):
        super(CVRPGATS, self).__init__(info)

        self.populations = [GATSPopulation(self.info, total_iters) for _ in range(num_populations)]
