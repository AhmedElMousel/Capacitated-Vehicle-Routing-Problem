...
class CVRPInfo(object):
    ...
    def make_random_solution(self, greedy=False):
        ...
        while unserviced:
            ...
            if greedy:
                ...
                i = min([i for i in range(len(unserviced))],key=lambda x: self.dist[cur_route[-1] if random.uniform(0, 1) < 0.9 else 1][unserviced[x]])
            node = unserviced[i]
    ...
    def random_swap(self,solution):
        route_i = random.randint(0,len(solution))
        r = copy.copy(solution[route_i])
        for i in range(2):
            x = random.randint(0,len(r))
            y = -1
            while x != y:
               y = random.randint(0,len(r))
            r[x] , r[y] = r[y] , r[x]
        new_routes = solution[:route_i] + [r] + solution[(route_i + 1):]
        return self.make_solution(new_routes)
    ...
    def visualise(self, solution , b , itr):
        plot1 = plt.figure(1)
        plt.plot(self.coords[1][0] ,self.coords[1][1] ,'s')
        j=1
        for l in solution.routes :
            if (len(l.route)>2):
                plt.plot([self.coords[l.route[i]][0] for i in range(len(l.route))],[self.coords[l.route[i]][1] for i in range(len(l.route))],'.-',label = 'Vehicle #'+str(j))
#            for i in range(len(l.route)):
#                plt.annotate(l.route[i],xy=(self.coords[l.route[i]][0],self.coords[l.route[i]][1] ))
            j=j+1
        #plt.legend(loc="upper right")
        plt.title("Best Solution", fontsize=20, fontweight='bold')       
        plot2 = plt.figure(2)
        print len(itr)
        print len(b)
        plt.plot(itr, b)
        ...
        plt.xlim(0, len(itr))
        plt.xticks(np.arange(min(itr), max(itr), int(len(itr)/5)), fontweight='bold')
        plt.yticks(fontweight='bold')
        
        plot3 = plt.figure(3)
        itr1 = itr[:int(len(itr)*0.2)]
        b1 = b[:int(len(itr)*0.2)]
        ...
        plt.ylabel("Distance", fontsize=18, fontweight='bold')
        plt.xlim(0, len(itr1))
        plt.xticks(np.arange(min(itr1), max(itr1),int(len(b1)/5)), fontweight='bold')
        plt.yticks(fontweight='bold')   
...
if __name__ == "__main__":
    ci = CVRPInfo("validate/X-n190-k8.vrp")
