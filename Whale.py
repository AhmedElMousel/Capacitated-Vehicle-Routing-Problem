# -*- coding: utf-8 -*-
"""
Created on Thu Jul  2 01:46:54 2020
...
class WhalePopulation(object):
    def __init__(self, info, total_iters):
        ...
        for x in [self.info.steep_improve_solution(self.info.make_random_solution(greedy=True)) for _ in range(800)]:
            heapq.heappush(self.chromosomes, (x.cost, x))
    ...
    def step(self):
        replace = 1
        for i in range(12):
            jc = self.chromosomes[random.randrange(0, 13)][1]
            ic = self.chromosomes[i][1]
            while ic.cost == jc.cost :
                jc = self.chromosomes[random.randrange(0, 13)][1]
            child = self.biggest_overlap_crossover(ic, jc)
            if random.uniform(0, 1) < 0.5:
                if random.uniform(0, 1) > (1-((self.iters/self.total_iters)**2)):
                    for _ in range(3):
                    ...
                    for _ in range(3):
                        c = self.biggest_overlap_crossover(jc, child)
                        self.info.refresh(c)
                        if c.cost < child.cost :
                            child = c
            else :
                for _ in range(3):
                    c = self.simple_random_crossover(jc, child)
                    self.info.refresh(c)
                    if c.cost < child.cost :
                        child = c
            ...
            replace += 1
        heapq.heapify(self.chromosomes)
        self.iters += 1
...
class CVRPWhale(CVRPAlgorithm):
    def __init__(self, info, num_populations, total_iters):
        super(CVRPWhale, self).__init__(info)

        self.populations = [WhalePopulation(self.info, total_iters) for _ in range(num_populations)]
