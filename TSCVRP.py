...
for i in range(Runs):
    ...
    while t < len(X0)-1:
        X_Swap = rnd_sol_1[:]
        X_Swap[0], X_Swap[t] = X_Swap[t], X_Swap[0]
        t += 1
        Store_all_Combinations = np.vstack((Store_all_Combinations, X_Swap))
    ...
    if len(Save_Solutions_Here) >= 1:
        ...
        if Solution_in_Hand in Tabu_List:  # If the solution is already in the tabu list
            while Solution_in_Hand[0] in Tabu_List[:, 0] and t < len(OF_Values_all_N):
                # The distance of the solution is the same as one in the tabu
                if Solution_in_Hand[0] < Best_So_Far[0]:  # If it is less, then...
                    ...
                    # If it wasn't less, the current solution becomes what was chosen in the list
        ...
        if len(Tabu_List) >= Length_of_Tabu_List:  # If the tabu list was full
            ...
            if Tabu_List[0, 0] != Solution_in_Hand[0]:
                ...
                for i in range(Diff):
                    ...
                    # Remove the last/oldest solution
            ...
            Tabu_List = np.vstack((Solution_in_Hand, Tabu_List))
            # If the tabu list was not full, put the solution in hand in it
